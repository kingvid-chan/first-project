					...
					/*
						弹框的个性化配置参数：
							coupon : {													弹框类型
								...
								sku0BtnTxt : '',										跳转到CONF.skuIds[0]按钮文案（与之类似的还有sku1BtnTxt、sku2BtnTxt...）
						...
						按钮类型有以下值：
							...
							jump0、jump1、jump2...	跳转到CONF.jumpUrls[0]、CONF.jumpUrls[1]、CONF.jumpUrls[2]...（在xview中，将先关闭xview，再在webview中打开CONF.jumpUrls[0]、CONF.jumpUrls[1]、CONF.jumpUrls[2]...；其他环境，唤起app并在app中打开CONF.jumpUrls[0]、CONF.jumpUrls[1]、CONF.jumpUrls[2]...，同时当前页直接跳转到CONF.jumpUrls[0]、CONF.jumpUrls[1]、CONF.jumpUrls[2]...）
							sku0、sku1、sku2...		跳转到CONF.skuIds[0]、CONF.skuIds[1]、CONF.skuIds[2]...（在xview中，将先关闭xview，再在webview中打开CONF.skuIds[0]、CONF.skuIds[1]、CONF.skuIds[2]...；其他环境，唤起app并在app中打开CONF.skuIds[0]、CONF.skuIds[1]、CONF.skuIds[2]...，同时当前页直接跳转到CONF.skuIds[0]、CONF.skuIds[1]、CONF.skuIds[2]...）
							shop0、shop1、shop2...	跳转到CONF.shopIds[0]、CONF.shopIds[1]、CONF.shopIds[2]...（在xview中，将先关闭xview，再在webview中打开CONF.shopIds[0]、CONF.shopIds[1]、CONF.shopIds[2]...；其他环境，唤起app并在app中打开CONF.shopIds[0]、CONF.shopIds[1]、CONF.shopIds[2]...，同时当前页直接跳转到CONF.shopIds[0]、CONF.shopIds[1]、CONF.shopIds[2]...）
							replay					再玩一次
							retry					重新发送抽奖请求
					 */
					default : {
						...
						popup[type] = function(info,noAnim){
							...
							conf.orders.forEach(function(name){
								switch(name){
									case 'icon':
										if(conf.icon != null){
											$(TPLS.icon).addClass('padt_p_icon_' + popup.parse(conf.icon,info)).appendTo($content);
										}
										break;
									case 'title':
										if(conf.title != null){
											findFromSelf($(TPLS.title).appendTo($content),'[padt-role="title"]').html(popup.parseLine(popup.parse(conf.title,info)));
										}
										break;
									case 'subtitle':
										if(conf.subtitle != null){
											findFromSelf($(TPLS.subtitle).appendTo($content),'[padt-role="subtitle"]').html(popup.parseLine(popup.parse(conf.subtitle,info)));
										}
										break;
									case 'pack':
										if(conf.pack && conf.pack.length && info && info.pack && info.pack.length){
											...
											info.pack.forEach(function(packItem){
												...
												if(conf.packItemClass){
													$packItem.addClass(popup.parse(conf.packItemClass,packItem));
												}
												conf.pack.forEach(function(packItemConf){
													...
													).html(popup.parseLine(popup.parse(packItemConf.val,packItem)));
												});
											});
										...
										if(conf.btns && conf.btns.length){
											...
											if(conf.btns && conf.btns.length){
												...
												conf.btns.forEach(function(btnType){
													var $btn = $(TPLS.btn),
														btnTxt = conf[btnType + 'BtnTxt'];
													if(btnTxt){
														findFromSelf($btn,'[padt-role="txt"]').html(popup.parseLine(popup.parse(btnTxt,info)));
													}
									...
									case 'prizes':
										if(conf.prizes && conf.prizes.length){
											...
											var setPrizeDomProp = function(type,$dom,val){
													switch(type){
														case 'html':
															$dom.html(popup.parseLine(popup.parse(val,info)));
															break;
														case 'class':
															$dom.addClass(popup.parse(val,info));
															break;
														default:
															$dom.attr(type,popup.parse(val,info));
													}
												};
									...
									case 'tips':
										if(conf.tips != null){
											findFromSelf(
												...
												'[padt-role="tips"]'
											).html(popup.parseLine(popup.parse(conf.tips,info)));
										}
										break;
									case 'link':
										if(conf.linkTxt != null && conf.linkType != null){
											findFromSelf(
												$(TPLS.linkWrap).appendTo($content),
												'[padt-role="link"]'
											).html(popup.parseLine(popup.parse(conf.linkTxt,info))).attr('padt-handler',conf.linkType);
										}
										break;
								}
							});
							if(conf.class){
								$popup.addClass(popup.parse(conf.class,info));
							}
							if(noAnim){
				...
				(function(handlerType){
					switch(handlerType){
						case 'share':
							 ...
							 * @param {Number | null} [shareKey = null] - 分享配置的键（为null表示不设置）
							 */
						...
						default:
							...
							/**
							 * 调用
							 * @param {String} [type = ''] - 调用该handler的类型
							 */
